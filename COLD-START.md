...
- **The radar loop.** The radar itself works and draws real NOAA imagery (Pass 14), but it shows **one frame, not an animation**. NOAA has about two hours of history and the app reads it correctly; the blocker is that Pass 13's alpha-swap loop does not repaint on the device. `RadarSource.frameCount` is 1 for that reason and says so in the code.
