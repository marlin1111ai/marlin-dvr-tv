...
- **A radar loop that does not hammer NOAA.** The animation itself works (Pass 15), but every step re-fetches every tile — about 2,300 requests a minute — and NOAA returned HTTP 403 under that load during testing. Nothing in the app is broken; what is missing is a small in-memory store of the tiles already fetched, which was outside Pass 15's scope. Until then the radar is best not left open for long stretches.
